...
static void server(void) {
  ...
  for (;;) {
    ...
    if (in > 0) {
      ...
      printf("Server: Trying to send the message %s, result is %d\n", msg, ret);
      sleep(40); // for debug purposes
    }
...
static void client(char* serverIp) {
  ...
  ret = sctp_sendmsg(conn_fd, (void *) msg, strlen(msg) + 1, NULL, 0, 0, 0, 0, 0, 0 );
  printf("Client: Trying to send the message %s, result is %d\n", msg, ret);
  if (ret < 0)
    die("sctp_sendmsg");

  sleep(40); // for debug purposed
